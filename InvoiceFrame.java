...
public class InvoiceFrame extends Frame{
	...
	public InvoiceFrame() {
		...
		for (int i=0; i<colNames.length; i++) {
			invoiceTableData += StringAligner.centerString(colNames[i], colWidth);
		}
		invoiceTableData += "\n";
		for (int i=0; i<products.size(); i++) {
			Product p = products.get(i);
			
			invoiceTableData += StringAligner.centerString(String.valueOf(i), colWidth);
			invoiceTableData += StringAligner.centerString(p.getName(), colWidth);
			invoiceTableData += StringAligner.centerString(String.valueOf(1), colWidth);
			invoiceTableData += StringAligner.centerString( "$" + String.valueOf(p.getPrice()), colWidth);
			invoiceTableData += "\n";
		}
